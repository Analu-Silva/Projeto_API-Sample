...
public class RegisterController {
	...
	@PostMapping
	public ResponseEntity<RegisterEntity> creatRegister(@RequestBody RegisterDTO registerDTO) throws Exception{
	...
	public ResponseEntity<List<RegisterEntity>> getRegisters() throws Exception {
		...
		//return ReponseEntity.status(200).body(registers);
		return ResponseEntity.ok(registers);
	}
	
	@GetMapping("/{id}")
	public ResponseEntity<RegisterEntity> getOnRegister(@PathVariable UUID id) throws Exception {
	...
	public ResponseEntity<String> handleException(Exception ex) {
		String menssage = ex.getMessage().replaceAll("/r/n", "");
